...
#TestTraegerClient
async def test_handle_tokens(traeger_client: TraegerTestClient, http: aioresponses) -> None:
    """test getting token"""
    http.post(api_token['url'], payload=api_token['resp'])
    traeger_client._api['username'] = "JohnyTraeger@traeger.com"
    traeger_client._api['password'] = "abc123"
...
async def test_handle_tokens_bad_user_pass(traeger_client: TraegerTestClient, http: aioresponses) -> None:
    """test getting token with bad PAR"""
    http.post(api_token['url'], payload={'error':'badpass'})
    traeger_client._api['username'] = ""
    traeger_client._api['password'] = ""
    response = await traeger_client.do_cognito()
    _LOGGER.warning("do cognito resp: %s", response)

async def test_handle_user(traeger_client: TraegerTestClient, http: aioresponses) -> None:
...
async def test_handle_user_bad(traeger_client: TraegerTestClient, http: aioresponses) -> None:
...
async def test_handle_mqtturl(traeger_client: TraegerTestClient, http: aioresponses) -> None:
    ...
    _LOGGER.warning("do cognito resp: %s", response)
    assert traeger_client._api['mqtt_url_expires'] not in [None, "", 0]
    assert traeger_client._api['mqtt_url'] not in [None, "", 0]

async def test_handle_mqtturl_bad(traeger_client: TraegerTestClient, http: aioresponses) -> None:
...
async def test_handle_cmd(traeger_client: TraegerTestClient, http: aioresponses) -> None:
    """test grill command"""
    http.post(api_token['url'], payload=api_token['resp'])
...
async def test_handle_cmd_bad(traeger_client: TraegerTestClient, http: aioresponses) -> None:
    """test grill command"""
    http.post(api_token['url'], payload={'error':'badpass'})
    http.post(api_commands['url'], payload={'error':'badtoken'})
    traeger_client._api['username'] = "JohnyTraeger@traeger.com"
    traeger_client._api['password'] = None
