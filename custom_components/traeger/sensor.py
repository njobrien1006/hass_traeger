...
async def async_setup_entry(hass, entry, async_add_devices):
    ...
    for grill in grills:
        ...
        async_add_devices(
            [
                PelletSensor(
                    client, grill["thingName"], "Pellet Level", "pellet_level"
                )
            ]
        )
        async_add_devices(
        ...
        async_add_devices(
            [
                GrillTimer(
                    client, grill["thingName"], "Cook Timer Start",
                    "cook_timer_start"
                )
            ]
        )
        async_add_devices(
            [
                GrillTimer(
                    client, grill["thingName"], "Cook Timer End",
                    "cook_timer_end"
                )
            ]
        )
        async_add_devices(
            [
                GrillState(
                    client, grill["thingName"], "Grill State", "grill_state"
                )
            ]
        )
        async_add_devices(
...
class PelletSensor(TraegerBaseSensor):
    ...
    def available(self):
        ...
        if self.grill_features is None:
            ...
            return True if self.grill_features["pellet_sensor_connected"
                                              ] == 1 else False
...
class HeatingState(TraegerBaseSensor):
    ...
    def state(self):
        ...
        min_cook_temp = (
            GRILL_MIN_TEMP_C
            if self.grill_units == TEMP_CELSIUS else GRILL_MIN_TEMP_F
        )
...
class ProbeState(TraegerBaseSensor):
    def __init__(self, client, grill_id, sensor_id):
        super().__init__(
            client, grill_id, f"Probe State {sensor_id}",
