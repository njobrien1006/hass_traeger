...
class CookCycNumberEntity(NumberEntity, TraegerBaseEntity):
    ...
    def value(self):
        ...
        if self.num_value > len(self.cook_cycle):
            ...
            self.num_value = 0
        if self.num_value > 0 and not (
            4 <= self.grill_state["system_status"] <= 6
        ):
        ...
        if self.num_value > 0 and self.num_value != self.old_num_value:  #Only hit once per step.
            ...
            if "time_set" in curstep:
                self.hass.async_create_task(
                    self.client.set_timer_sec(
                        self.grill_id, round(curstep["time_set"])
                    )
                )
            ...
            if "set_temp" in curstep:
                self.hass.async_create_task(
                    self.client.set_temperature(
                        self.grill_id, round(curstep["set_temp"])
                    )
                )
            if "smoke" in curstep:
