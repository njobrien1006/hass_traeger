...
async def async_setup_entry(hass, entry, async_add_devices):
    ...
    for grill in grills:
        ...
        async_add_devices(
            [
                TraegerSuperSmokeEntity(
                    client, grill["thingName"], "smoke", "Super Smoke Enabled",
        ...
        async_add_devices(
            [
                TraegerSwitchEntity(
                    client, grill["thingName"], "keepwarm", "Keep Warm Enabled",
                    "mdi:beach", 18, 19
                )
            ]
        )
...
class TraegerSwitchEntity(TraegerBaseSwitch):
    ...
    async def async_turn_on(self, **kwargs):
        """Set new Switch Val."""
        if GRILL_MODE_IGNITING <= self.grill_state['system_status'
                                                  ] <= GRILL_MODE_CUSTOM_COOK:
...
class TraegerSuperSmokeEntity(TraegerSwitchEntity):
    ...
    def available(self):
        if self.grill_state is None:
            ...
            if GRILL_MODE_IGNITING <= self.grill_state[
                'system_status'] <= GRILL_MODE_CUSTOM_COOK:
                return True if self.grill_features["super_smoke_enabled"
                                                  ] == 1 else False
        return False
